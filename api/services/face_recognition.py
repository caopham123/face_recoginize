...
def register_member(target_id: int = None, target_email = None, target_name = None, target_image = None):
    ...
    if target_image is not None:
        _, embedding = face_recognition.detection.dectect_face(target_image)

        # Not found faces on image
        if embedding is None:
            return False

        response = trainer.register_member(target_id, target_email, target_name, embedding)
        face_recognition = FaceRecognition() # Load db
        return response
